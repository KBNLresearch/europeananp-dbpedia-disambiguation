...
# adapted string similarity: combines jaro-winkler distance with number of
# common terms in both strings
def _stringSimilarity(a, b):
    if a and b and len(a) > 0 and len(b) > 0:
        ...
        if (sa and sb and len(sa) > 0 and len(sb) > 0):
            jarowinkler = fuzzycomp.jaro_winkler(a.encode('utf-8'),
                                                 b.encode('utf-8'),
                                                 0.1)
            wordoverlap = (float(len(intersect))
                           / float(max(len(sa), len(sb))))
            return jarowinkler * wordoverlap
        else:
            return 0.0
    else:
        return 0.0
...
def linkEntity(namedEntityString):
    ...
    labelQuery = "label_en:\"" + cleaned + "\"^2000 " + \
        " ".join(["label_en:" + elt for elt in cleaned.split(" ")])
    redirectLabelQuery = "redirectLabel:\"" + cleaned + "\"^2000 " + \
        " ".join(["redirectLabel:" + elt for elt in cleaned.split(" ")])
    try:
        result = s.raw_query(q="\
  		((" + labelQuery + ") OR (" + redirectLabelQuery + ")) \
      AND _val_:inlinks^4 \
      AND (schemaorgtype:Person^10 \
      OR schemaorgtype:Place \
      OR schemaorgtype:Organization)",
                             fq="schemaorgtype:Person \
                              OR schemaorgtype:Place \
                              OR schemaorgtype:Organization",
                             fl="* score",
                             rows=5,
    ...
    for d in jsonResult["response"]["docs"]:
        ...
        if (d.get("score") / maxScore) > CUTOFF_RELEVANCY:
            ...
            sumLabels[d.get("id")] = [
                (_cleanedLabel(d.get("label_en")), d.get("score"))]
            for l in labels:
                sumLabels[d.get("id")].append(
                    (_cleanedLabel(l), d.get("score")))
    ...
    for d in sumLabels.keys():
        for l in sumLabels.get(d):
            ...
            if (similarityScore > CUTOFF_SIMILARITY):
                ...
                labelScore = similarityScore * \
                    math.sqrt(relativeRelevancyScore)
                if labelScore > score:
                    bestMatch = d
